...
Actions.prototype.setup = function () {
    ...
    this._executePararell("setup", [this.config], isVerbose);
};
    ...
    async.mapSeries(
        ...
        function (sessionData, callback) {
            ...
            var taskList = taskListsBuilder.deploy(
                bundlePath, env,
                deployCheckWaitTime, appName, self.config.appDirectory, self.config.appIdentifier, self.config.git, isVerbose);
