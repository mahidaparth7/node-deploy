...
Actions.prototype.deploy = function () {
    ...
    cloneApp(host, appPath, function (err) {
            ...
            async.mapSeries(
                ...
                function (sessionData, callback) {
                    ...
                    var taskList = taskListsBuilder.deploy(
                        bundlePath, env,
                        deployCheckWaitTime, appName, self.config.appDirectory, self.config.git);
    ...
    for (var os in self.sessionsMap) {
        ...
        sessionsInfo.sessions.forEach(function (session) {
            ...
            if (os == 'linux') {
                ...
                var command = 'pm2 logs ' + self.config.appName + ' --lines ' + logLines;
            } else {
                throw "Unknown OS: " + os;
            }
